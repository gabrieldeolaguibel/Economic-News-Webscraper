...
actual_greater_events = ['CB Consumer Confidence', 'ADP Non-Farm Employment Change', 'ISM Manufacturing PMI', 'JOLTS Job Openings', 'Federal Funds Rate']

# Set 'Effect' column value when event is in the list
df.loc[df['Event'].isin(actual_greater_events), 'Effect'] = 'Actual greater than Forecast is good for the economy'

df

import requests
# Identify the rows that have data in the 'Actual' column
actual_data_rows = df[df['Actual'] != '']
...
for index, row in actual_data_rows.iterrows():
  ...
  requests.post(url='https://hooks.slack.com/services/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',  json={
	"blocks": [
		{
			"type": "header",
			"text": {
		...
		},
		{
			...
			"elements": [
				...
				{
					"type": "mrkdwn",
					"text": "Economic scraper by *Gabriel de Olaguibel*"
				}
			]
		}
	]
...
for index, row in actual_data_rows.iterrows():
    # Create a string with all the data from the row
  data_string = 'Date : {}, Time : {}, Currency : {}, Event : {}, Actual : {}, Forecast : {}, Previous : {}'.format(row['Date'], row['Time'], row['Currency'], row['Event'], row['Actual'], row['Forecast'], row['Previous'])
  # Send the data from the 'Actual' column to the desired channel in Slack
  requests.post(url='https://hooks.slack.com/services/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', json={'text': row['Actual']})

print(actual_data_rows)
